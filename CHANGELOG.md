...
* If upgrading to Consul 1.8.0 and using Consul Connect you will need to first upgrade consul-k8s to 0.16.0 (by setting `global.imageK8S: hashicorp/consul-k8s:0.16.0`) and re-roll your Connect pods so they get re-injected. This is required because we were previously setting a health check incorrectly that now fails on Consul 1.8.0. If you upgrade to 1.8.0 without upgrading to consul-k8s 0.16.0 first and re-rolling your connect pods, then the connect pods will fail their health checks and no traffic will be routed to them.
