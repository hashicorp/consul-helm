...
* Add the ability to manually specify a k8s secret containing server-cert via the value `server.serverCert.secretName`. Coupled with the autoEncrypt feature, this removes the need for the tls-init job and can be run in GitOps without having to rely on Helm hooks. [[GH-1024](https://github.com/hashicorp/consul-helm/pull/1046)]
