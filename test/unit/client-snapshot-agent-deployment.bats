...
@test "client/SnapshotAgentDeployment: adds extra volume" {
  cd `chart_dir`
  ...
  local object=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraVolumes[0].type=configMap' \
      --set 'client.snapshotAgent.extraVolumes[0].name=foo' \
      . | tee /dev/stderr |
...
@test "client/SnapshotAgentDeployment: adds extra secret volume" {
  cd `chart_dir`
  ...
  local object=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraVolumes[0].type=secret' \
      --set 'client.snapshotAgent.extraVolumes[0].name=foo' \
      . | tee /dev/stderr |
      yq -r '.spec.template.spec.volumes[] | select(.name == "userconfig-foo")' | tee /dev/stderr)
...
@test "client/SnapshotAgentDeployment: adds loadable volume" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraVolumes[0].type=configMap' \
      --set 'client.snapshotAgent.extraVolumes[0].name=foo' \
      --set 'client.snapshotAgent.extraVolumes[0].load=true' \
      . | tee /dev/stderr |
      yq -r '.spec.template.spec.containers[0].command | map(select(test("/consul/userconfig/foo"))) | length' | tee /dev/stderr)
  [ "${actual}" = "1" ]
}

@test "client/SnapshotAgentDeployment: adds readonly volume" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraVolumes[0].type=configMap' \
      --set 'client.snapshotAgent.extraVolumes[0].name=foo' \
      --set 'client.snapshotAgent.extraVolumes[0].readOnly=true' \
      . | tee /dev/stderr |
      yq -r '.spec.template.spec.containers[0].volumeMounts[] | select(.name == "userconfig-foo") | .readOnly' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}

@test "client/SnapshotAgentDeployment: adds custom path volume" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraVolumes[0].type=configMap' \
      --set 'client.snapshotAgent.extraVolumes[0].name=foo' \
      --set 'client.snapshotAgent.extraVolumes[0].mountPath=/foo/bar' \
      . | tee /dev/stderr |
...
@test "client/SnapshotAgentDeployment: extraContainers is not set by default" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
...
@test "client/SnapshotAgentDeployment: extraContainers is added" {
  cd `chart_dir`
  ...
  local object=$(helm template \
      -x templates/client-snapshot-agent-deployment.yaml  \
      --set 'client.snapshotAgent.enabled=true' \
      --set 'client.snapshotAgent.extraContainers[0].name=vault-agent' \
      --set 'client.snapshotAgent.extraContainers[0].image=vault:latest' \
      --set 'client.snapshotAgent.extraContainers[0].args[0]=-config=/etc/vault/config.hcl' \
      . | tee /dev/stderr |
